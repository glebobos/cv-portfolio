...
const Certifications = ({ isPrintView = false }: CertificationsProps) => {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid gap-8">
          {Object.entries(certificationGroups).map(([issuer, certs], groupIndex) => {
                ...
                <div className="p-6">
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {certs.map((cert, index) => {
                      ...
                      if (isPrintView) {
                        return <div key={index} className="group">{certContent}</div>;
                      }
            ...
            if (isPrintView) {
              return (
                <div key={issuer} className="bg-white rounded-2xl shadow-lg overflow-hidden">
                  {groupContent}
                </div>
              );
            }
